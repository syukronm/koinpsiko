...
case "upcatad":
	if($_GET['kuy']=="")//awal
	{
		$sql= "SELECT * FROM tab_cpu ORDER BY id_cpu ASC";
		$rs=$db->Execute($sql);	
		...
		$ekp->assign("view", $view);
		$ekp->assign("manip", "awal");
	}
	...
	{	
		...
		$ekp->assign("manip", "tambah");
		$ekp->assign("tahun", $tahun);
	}
	...
	elseif($_GET['kuy']=="u")//ubah
	{
		...
		$ekp->assign("index_data", $index_data);
		$ekp->assign("tahunn", $tahunn);
		$ekp->assign("kode_barang", $kode_barang);
		$ekp->assign("pemilik", $pemilik);
		$ekp->assign("unit", $unit);
		$ekp->assign("pcname", $pcname);
		$ekp->assign("tahun", $tahun);
		$ekp->assign("brand_cpu", $brand_cpu);
		$ekp->assign("hdd_tipe", $hdd_tipe);
		$ekp->assign("ukuran_hdd", $ukuran_hdd);
		$ekp->assign("tipe_ram", $tipe_ram);
		$ekp->assign("ukuran_ram", $ukuran_ram);
		$ekp->assign("prossesor", $prossesor);
		$ekp->assign("os", $os);
		$ekp->assign("ada_lan", $ada_lan);
		$ekp->assign("ip_lan", $ip_lan);
		$ekp->assign("ada_wifi", $ada_wifi);
		$ekp->assign("brand_monitor", $brand_monitor);
		$ekp->assign("ukuran_monitor", $ukuran_monitor);
		$ekp->assign("kondisi", $kondisi);
		$ekp->assign("foto", $foto);
		$ekp->assign("ket", $ket);
		$ekp->assign("manip", "ubah");
	}
...
case "potlap":
	...
	elseif($_GET['kuy']=="u")//ubah
	{
		...
		$ekp->assign("nama_lembaga", $nama_lembaga);
		$ekp->assign("lokasi", $lokasi);
		$ekp->assign("program_studi", $program_studi);
		$ekp->assign("tgl_lulus", $tgl_lulus);
		$ekp->assign("no_ijazah", $no_ijazah);
		$ekp->assign("index_data", $index_data);
		$ekp->assign("manip", "ubah");
	}
	elseif($_GET['kuy']=="uok")//ubah_ok
	...
	$ekp->assign("isi","riwayat_karier.tpl");
break;
//======================================================================
...
case "nangiraj":
	...
	$sql = "SELECT
			...
			INNER JOIN 
				tbl_ket_golongan ON tbl_pegawai.id_ket_golongan = tbl_ket_golongan.id_ket_golongan 
			WHERE tbl_pegawai.tipe=2 ORDER BY tbl_pegawai.nama ASC";
	$rs=$db->Execute($sql);
	...
	if ($halaman != 1)
	{
		$navigasi = "<a href='pegawai.php?action=tendik&halaman=".($halaman-1)."'>&laquo; Prev</a> ";
	}
	// Tampilan Halaman
	for ($i=0;$i<=$range;$i++)
	{
		if ($barisan_angka <= $totalhalaman)
		{
			if ($barisan_angka == $halaman)
			{
				...
				$navigasi .= "<a href='pegawai.php?action=tendik&halaman=$barisan_angka'>$barisan_angka</a>";
				$navigasi .= " ";
			}
				$barisan_angka++;
		}
	...
	if (($halaman != $totalhalaman) and ($jml!= 0))
	{
		$navigasi .= " <a href='pegawai.php?action=tendik&halaman=".($halaman+1)."'>Next &raquo;</a>";
	}
	while($row=$rs->FetchRow())
	{
		$view[]=$row;
